...
namespace Engine
{
	...
	void Model::Load(std::string const& path)
	{
		...
		m_animation = CreatePtr<ModelData::Animation>(scene->mAnimations[0]);
		m_rootNode = CreatePtr<ModelData::Node>(scene->mRootNode, m_animation);
	...
	Ptr<ModelData::Mesh> Model::ProcessMesh(const aiMesh* mesh, const aiMaterial* material)
	{
		...
		return CreatePtr<ModelData::Mesh>(vertices, indices, textures);
	}

	std::vector<Ptr<Texture>> Model::LoadMaterialTextures(const aiMaterial* material, aiTextureType type, TextureType textureType)
	{
	...
	void Model::ExtractBoneWeightForVertices(std::vector<Vertex>& vertices, const aiMesh* mesh)
	{
		for (int boneIndex = 0; boneIndex < mesh->mNumBones; ++boneIndex)
		{
			aiBone* aiBone = mesh->mBones[boneIndex];
			std::string boneName = aiBone->mName.C_Str();
			Ptr<ModelData::Bone>& bone = m_animation->GetBoneByName(boneName);
	...
	void Model::CalculateBoneTransform(const Ptr<ModelData::Node> node, glm::mat4 parentTransform)
	{
		glm::mat4 globalTransformation = parentTransform * node->transformation;
		Ptr<ModelData::Bone> bone = node->bone;
		if (bone != nullptr)
		{
