...
namespace Engine
{
		...
		, m_isDeleted(false)
	{
		...
		{
			m_closestPointToOrigin = GetClosestPointToOrigin(originEnclosed);
			m_closestDistanceToOriginSquare = glm::length2(m_closestPointToOrigin);
		}
	}

	const glm::dvec3 GJK3DTriangle::GetClosestPointToOrigin(bool originEnclosed) const
	{
		if (originEnclosed)
		{
			double delta = glm::dot(m_normal, m_vectorA);

			return m_normal * (delta / m_normalLengthSquare);
		}
		else
		{
			glm::dmat3 matrix(m_vectorA, m_vectorB, m_vectorC);
			glm::dvec3 barycentric = GetBarycentric();

			return matrix * barycentric;
		}
	}

	const glm::dvec3 GJK3DTriangle::GetBarycentric() const
	{
