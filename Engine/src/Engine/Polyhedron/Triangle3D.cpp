...
namespace Engine
{
	Triangle3D::Triangle3D(const glm::dvec3& pointA, const glm::dvec3& pointB, const glm::dvec3& pointC)
	...
	{
		glm::dvec3 u = m_pointA - m_pointB;
		glm::dvec3 v = m_pointA - m_pointC;
	...
	void Triangle3D::SetClosestPointToOrigin(bool originEnclosed)
	{
		...
		if (glm::length2(m_normal) > NUMERIC_EPSILON * NUMERIC_EPSILON)
		{
			if (originEnclosed)
			{
				double delta = glm::dot(m_normal, (glm::dvec3)m_pointA);
	...
	glm::dvec3 Triangle3D::CalcBarycentric(bool originEnclosed)
	{
		...
		glm::dvec3 direction = glm::cross(u, (glm::dvec3)m_pointA);
		double gamma = glm::dot(direction, m_normal) / t;
		direction = glm::cross((glm::dvec3)m_pointA, v);
		...
		if (!originEnclosed)
		{
			...
			{
				t = glm::dot((glm::dvec3)m_pointA, u);
				if ((gamma < 0.0) && (t > 0.0))
				{
				...
				{
					gamma = std::min(1.0, std::max(0.0, glm::dot((glm::dvec3)m_pointA, v) / glm::length2(v)));
					alpha = 1.0 - gamma;
					beta = 0.0;
				}
			}
			...
			{
				glm::dvec3 w = m_pointB - m_pointC;
				t = glm::dot((glm::dvec3)m_pointB, w);
				if ((alpha < 0.0) && (t > 0.0))
				{
				...
				{
					alpha = std::min(1.0, std::max(0.0, -glm::dot((glm::dvec3)m_pointB, u) / glm::length2(u)));
					beta = 1.0 - alpha;
					gamma = 0.0;
				}
			}
			...
			{
				glm::dvec3 w = m_pointB - m_pointC;
				t = -glm::dot((glm::dvec3)m_pointC, v);
				if ((beta < 0.0) && (t > 0.0))
				{
				...
				{
					beta = std::min(1.0, std::max(0.0, -glm::dot((glm::dvec3)m_pointC, w) / glm::length2(w)));
					gamma = 1.0 - beta;
					alpha = 0.0;
				}
			}
	...
	{
		glm::dmat3 matrix(m_pointA, m_pointB, m_pointC);
